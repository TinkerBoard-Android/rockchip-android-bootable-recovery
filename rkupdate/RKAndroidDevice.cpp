...
void CRKAndroidDevice::load_volume()
{
    int i;
    int ret;

    int emmcState = getEmmc();
    if (emmcState == 2) {
...
bool CRKAndroidDevice::EraseSparseRegion(const char* volume,const char* directory)
{
	...
	if (v == NULL)
	{
		m_pLog->Record(_T("unknown volume \"%s"),volume);
	...
	if (strcmp(v->fs_type, "ext4") == 0 || strcmp(v->fs_type, "f2fs") == 0)
	{
		...
		if (v->key_loc != NULL && v->key_loc[0] == '/')
		{
			...
			if (fd < 0)
			{
				m_pLog->Record(_T("format_volume: failed to open %s"),v->key_loc);
				//printf("format_volume: failed to open %s\n", v->key_loc);
				bSuccess = false;
			}
			m_pLog->Record(_T("wipe_block_device"));
		...
		if (strcmp(v->fs_type, "ext4") == 0)
		{
			...
			if(result<0)
				bSuccess = false;
			#else
			m_pLog->Record(_T("System is sparse, erase system partition, but make_ext4fs_directory is not supported! SHOULDN'T be here! \n"));
			bSuccess = false;
			#endif
		}
		else
		{   /* Has to be f2fs because we checked earlier. */
			...
			{
				m_pLog->Record(_T("format_volume: crypt footer + negative length (%zd) not supported on %s\n"), length, v->fs_type);
				bSuccess = false;
			}
			if (length < 0)
			{
				m_pLog->Record(_T("format_volume: negative length (%zd) not supported on %s\n"), length, v->fs_type);
				bSuccess = false;
			}
			char *num_sectors;
			if (asprintf(&num_sectors, "%zd", length / 512) <= 0)
			{
				m_pLog->Record(_T("format_volume: failed to create %s command for %s\n"), v->fs_type, v->blk_device);
				bSuccess = false;
			}
